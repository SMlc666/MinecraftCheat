...
// 自定义颜色渐变函数（左右渐变）
ImU32 HorizontalGradientColor(float xPos, float maxWidth) {
...
// 绘制背景板
void DrawBackground(ImDrawList *drawList, float startX, float currentY, float maxWidth,
                    float totalHeight) {
  ...
  drawList->AddRectFilled(
      ...
      ImVec2(startX + maxWidth + bgPadding.x, currentY + totalHeight + bgPadding.y), bgColor, 12.0f,
      ImDrawFlags_RoundCornersAll);
}

// 绘制模块列表（左右渐变）
void DrawModuleList(ImDrawList *drawList, const std::vector<ModuleDisplayInfo> &modules,
                    float startX, float currentY, float maxWidth) {
    ...
    if (Arraylist::shadowIntensity > 0.01f) {
      for (int s = 0; s < 5; ++s) {
        ...
        const ImU32 shadowColor =
            IM_COL32(0, 0, 0, static_cast<int>(220 * Arraylist::shadowIntensity));
...
public:
  ...
  void Add(const std::string &message, ImU32 color = IM_COL32(255, 255, 255, 255)) {
    ...
    // 添加新通知
    notifications.push_back({message, static_cast<float>(ImGui::GetTime()), Notifications::duration,
                             color, 0.0f, yPos, yPos});
  }
  ...
  void Draw(ImDrawList *drawList) {
    ...
    for (auto &n : notifications) {
      ...
      const float scale = Notifications::enableScale ? 1.0f + (Notifications::maxScale - 1.0f) *
                                                                  (1.0f - std::abs(animT - 1.0f))
                                                     : 1.0f;
      ...
      // 背景矩形位置
      const ImVec2 bgMin(screenWidth - bgWidth - Notifications::margin.x + slideOffset,
                         verticalPos);
      const ImVec2 bgMax(screenWidth - Notifications::margin.x + slideOffset,
  ...
  void Update() {
    ...
    for (auto it = notifications.begin(); it != notifications.end();) {
      ...
      if (elapsed < Notifications::fadeTime) {
        ...
        n.animProgress =
            1.0f + (elapsed - (n.duration - Notifications::fadeTime)) / Notifications::fadeTime;
      }
...
cheat::Hud::Hud() : Module("Hud", MenuType::RENDER_MENU, ConfigData) {
  ...
  setOnDraw([](Module *module) {
    ...
    if (currentTime - lastSystemNotify > 30.0f) {
      ...
      NotificationManager::Get().Add("系统状态: " + std::to_string(activeCount) + "/" +
                                         std::to_string(ModuleManager::getModules().size()) +
                                         " 模块激活",
