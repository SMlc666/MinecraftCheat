...
namespace alias {
//NOLINTBEGIN
template <size_t Align, size_t Size> struct UntypedStorage {
...
template <size_t Align, size_t Size, class T> struct TypedStorageImpl {
  alignas(Align) std::byte data[Size];

  template <class... Args> constexpr TypedStorageImpl(Args &&...args) {
    std::construct_at(this->operator->(), std::forward<Args>(args)...);
  }

  TypedStorageImpl(TypedStorageImpl const &other) {
    std::construct_at(this->operator->(), other.get());
  }

  TypedStorageImpl(TypedStorageImpl &&other) {
    std::construct_at(this->operator->(), std::move(other.get()));
  }

  template <class U> constexpr T &operator=(U &&u) {
...
template <size_t A, size_t S, class T> struct TypedStorageType {
  using Type = TypedStorageImpl<A, S, typename std::remove_cv_t<T>>;
};
...
template <size_t A, size_t S, class T> struct TypedStorageType<A, S, std::unique_ptr<T>> {
  using Type = std::unique_ptr<T>;
};
template <size_t A, size_t S, class T> struct TypedStorageType<A, S, std::shared_ptr<T>> {
  using Type = std::shared_ptr<T>;
};
template <size_t A, size_t S, class T> struct TypedStorageType<A, S, std::weak_ptr<T>> {
  using Type = std::weak_ptr<T>;
};

template <size_t A, size_t S, class T> using TypedStorage = TypedStorageType<A, S, T>::Type;
//NOLINTEND
} // namespace alias