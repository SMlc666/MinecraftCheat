...
#include "game/minecraft/world/level/block/LevelEvent.hpp"
#include <string>
...
public:
  ...
  virtual void sendServerLegacyParticle(ParticleType name, class Vec3 const &pos,
                                        class Vec3 const &dir, int data);

  virtual void addParticleEffect(const HashedString &effect, const Actor &actor,
                                 const HashedString &locator, const Vec3 &emitterOffset,
                                 const MolangVariableMap &molangVariables);

  virtual void addTerrainParticleEffect(const BlockPos &pos, const Block &block,
                                        const Vec3 &emitterPosition, float intensity,
                                        float velocityScalar, float emitterRadius);

  virtual void addTerrainSlideEffect(const BlockPos &pos, const Block &block,
                                     const Vec3 &emitterPosition, float intensity,
                                     float velocityScalar, float emitterRadius);

  virtual void addBreakingItemParticleEffect(const Vec3 &pos, ParticleType type,
                                             const ResolvedItemIconInfo &);

  virtual void playMusic(std::string const &name, class Vec3 const &pos, float songOffset,
                         float volume);
  ...
  virtual void takePicture(cg::ImageBuffer &outImage, Actor *camera, Actor *target,
