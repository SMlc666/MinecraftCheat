...
public:
  ...
  // Constructors
  [[nodiscard]] constexpr HashedString(std::nullptr_t = nullptr) noexcept
      : mStrHash(0), mLastMatch(nullptr) {
  }

  [[nodiscard]] constexpr HashedString(uint64 h, char const *mStr) noexcept
      : mStrHash(h), mStr(mStr), mLastMatch(nullptr) {
  }

  [[nodiscard]] constexpr HashedString(std::string_view mStr) noexcept
      : mStrHash(computeHash(mStr)), mStr(mStr), mLastMatch(nullptr) {
  }

  [[nodiscard]] constexpr HashedString(std::string &&s) noexcept
      : mStrHash(computeHash(s)), mStr(std::move(s)), mLastMatch(nullptr) {
  }
  [[nodiscard]] constexpr HashedString(char const *mStr) noexcept
      : HashedString(std::string{mStr}) {
  }

  [[nodiscard]] constexpr HashedString(HashedString const &other) noexcept
      : mStrHash(other.mStrHash), mStr(other.mStr), mLastMatch(nullptr) {
  }

  [[nodiscard]] constexpr HashedString(HashedString &&other) noexcept
      : mStrHash(other.mStrHash), mStr(std::move(other.mStr)), mLastMatch(other.mLastMatch) {
    other.mStrHash = 0;
  ...
  [[nodiscard]] constexpr uint64 getHash() const noexcept {
    return mStrHash;
  }
  ...
  operator<=>(HashedString const &other) const noexcept {
    if (mStrHash != other.mStrHash) {
  ...
  // Convertors
  [[nodiscard]] constexpr operator std::string const &() const {
    return mStr;
  }
