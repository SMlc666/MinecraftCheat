...
struct map_info_t {
  ...
  std::string perms;
  std::string name;
  map_info_t() : start(0), end(0), offset(0) {
  }
  map_info_t(uintptr_t _start, uintptr_t _end, uintptr_t _offset, const std::string &_perms,
             const std::string &_name)
      : start(_start), end(_end), offset(_offset), perms(_perms), name(_name) {
  }

  inline bool operator==(const map_info_t &rhs) const {
...
struct ref_info_t {
  ...
  ref_info_t(const map_info_t &_map_info, uintptr_t _ptr) : map_info(_map_info), ptr(_ptr) {
  }

  inline bool operator==(const ref_info_t &rhs) const {
    return ptr == rhs.ptr;
  }
  inline bool operator!=(const ref_info_t &rhs) const {
    return ptr != rhs.ptr;
...
static inline bool is_page_valid(void *p) {
  ...
  return (p != nullptr && mincore((void *)(uintptr_t(p) & ~(page_size - 1)), page_size, vec) == 0 &&
          (vec[0] & 1) != 0);
}
...
bool iMsgCapture::findByMallocScan(const std::set<int> &sockets,
                                   const std::vector<std::string> &gameActivites, int *outFd,
                                   iMsgEvent **outMsgPtr) const {
...
EIMSG_STATUS iMsgCapture::runLoop(const std::vector<std::string> &gameActivites,
                                  bool retryOnFailure, int retrySleepSec) {
  EIMSG_STATUS lastStatus = IMSG_STATUS_NONE;
  char b = 0;
  int n = 0;
  for (;;) {
    if (lastStatus != IMSG_STATUS_NONE && lastStatus != IMSG_STATUS_OK &&
        lastStatus != IMSG_STATUS_WOULD_BLOCK) {
      if (!retryOnFailure)
        break;
    ...
    if (n < 0) {
      ...
      if (err != EAGAIN && err != EWOULDBLOCK) {
        iMSG_LOGE("runLoop: Consumer socket(%d) error(%d): \"%s\".", _consumerFd, err,
                  strerror(err));
...
void iMsgCapture::runThread(const std::vector<std::string> &gameActivites, bool retryOnFailure,
                            int retrySleepSec) {
  std::thread(
      [](iMsgCapture *self, const std::vector<std::string> &gameActivites, bool retryOnFailure,
         int retrySleepSec) {
      ...
      this, gameActivites, retryOnFailure, retrySleepSec)
      .detach();
}
...
static ssize_t syscall_vm_read(pid_t pid, const iovec *lvec, unsigned long liovcnt,
                               const iovec *rvec, unsigned long riovcnt, unsigned long flags) {
...
inline size_t vm_read(uintptr_t address, void *buffer, size_t len) {
  ...
  struct iovec rvec {
    .iov_base = reinterpret_cast<void *>(address), .iov_len = len
  };
...
std::vector<ref_info_t> _mallocScan(const void *data, size_t size) {
  ...
  auto findInRange = [](uintptr_t start, uintptr_t end, const void *data,
                        size_t dataSize) -> uintptr_t {
  ...
  _walkMaps([&](const map_info_t &map) -> bool {
    if (!(((map.end - map.start) > (128 * 1024)) && map.offset == 0 && !map.perms.compare("rw-p") &&
          !map.name.compare(mallocPathname)))
      return false;
    ...
    do {
      uintptr_t found =
          findInRange(curr_search_address, reinterpret_cast<uintptr_t>(buf.data()) + n, data, size);
      if (!found)
        break;
...
std::vector<int *> _scanChannelFds(const std::set<int> &sockets, const std::string &activity) {
  ...
  for (const auto &channelNamePtr : channelNamePtrs) {
    ...
    iMSG_LOGI("mallocScan: Scanning channel name at %p [%p - %p \"%s\"]", (void *)str_start,
              (void *)channelNamePtr.map_info.start, (void *)channelNamePtr.map_info.end,
              channelNamePtr.map_info.name.c_str());
    ...
    for (const auto &channelNameRef : channelNameRefs) {
      ...
      iMSG_LOGI("mallocScan: Found valid channel socket fd at %p [%p - %p \"%s\"]", _pFd,
                (void *)channelNameRef.map_info.start, (void *)channelNameRef.map_info.end,
                channelNameRef.map_info.name.c_str());
...
uintptr_t _scanInputMessage(int *pFd) {
  ...
  for (const auto &ref1 : sp_channel_refs_tmp) {
    for (const auto &ref2 : sp_channel_refs_tmp) {
      ...
      if (dist < 0x1000) {
        sp_channel_refs.erase(std::remove(sp_channel_refs.begin(), sp_channel_refs.end(), ref1),
                              sp_channel_refs.end());
        sp_channel_refs.erase(std::remove(sp_channel_refs.begin(), sp_channel_refs.end(), ref2),
                              sp_channel_refs.end());
      }
    }
  }
  ...
  for (const auto &ref : sp_channel_refs) {
    ...
    iMSG_LOGI("mallocScan: sp_channel ref %p [%p - %p \"%s\"]", (void *)ref.ptr,
              (void *)ref.map_info.start, (void *)ref.map_info.end, ref.map_info.name.c_str());
