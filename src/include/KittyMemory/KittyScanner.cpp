...
uintptr_t findInRange(const uintptr_t start, const uintptr_t end, const char *pattern,
...
uintptr_t findHexFirst(const uintptr_t start, const uintptr_t end, std::string hex,
                       const std::string &mask) {
  if (start >= end || mask.empty() || !KittyUtils::String::ValidateHex(hex))
    return 0;
...
std::vector<uintptr_t> findIdaPatternAll(const uintptr_t start, const uintptr_t end,
...
std::vector<uintptr_t> findDataAll(const uintptr_t start, const uintptr_t end, const void *data,
                                   size_t size) {
  std::vector<uintptr_t> list;
...
RegisterNativeFn findRegisterNativeFn(const class ElfScanner &elf, const std::string &name) {
  ...
  string_loc = KittyScanner::findDataFirst(
      elf.baseSegment().startAddress, elf.baseSegment().endAddress, name.data(), name.length());
  if (!string_loc) {
  ...
  for (auto &it : elf.segments()) {
    if (it.is_rw) {
      ...
      KITTY_LOGD("findRegisterNativeFn: String at (%p) referenced at %p", (void *)string_loc,
                 (void *)string_xref);
...
ElfScanner::ElfScanner(uintptr_t elfBase, const std::vector<KittyMemory::ProcMap> &maps) {
  ...
  if (!_stringTable || !_symbolTable || !_strsz || !_syment) {
    ...
    KITTY_LOGD("ElfScanner: elfBase: %p | strtab=%p | symtab=%p | strsz=%p | syment=%p",
               (void *)elfBase, (void *)_stringTable, (void *)_symbolTable, (void *)_strsz,
               (void *)_syment);
    return;
  }
...
uintptr_t ElfScanner::findSymbol(const std::string &symbolName) const {
  ...
  if (_gnuHashTable) {
    const auto *sym = KittyUtils::Elf::GnuHash::LookupByName(
        _gnuHashTable, _symbolTable, _stringTable, _syment, _strsz, symbolName.c_str());
    if (sym && sym->st_value) {
  ...
  if (_elfHashTable) {
    const auto *sym = KittyUtils::Elf::ElfHash::LookupByName(
        _elfHashTable, _symbolTable, _stringTable, _syment, _strsz, symbolName.c_str());
    if (sym && sym->st_value) {
      return get_sym_address(sym);
    }
  }
...
std::vector<ElfScanner> ElfScanner::getAllELFs() {
  ...
  for (auto &it : maps) {
    ...
    if (!it.readable || it.offset != 0 || it.isUnknown() || it.inode == 0 || !it.is_private ||
        !it.isValidELF())
      continue;
